...
app.get('/getHotelDetails',function(req, res) {
    ...
    mongo.connect(mongoURL, function(){
        ...
        coll.find({City: hotelPlace, hotelsDateFrom: {$lt:hotelsDateFrom[0]},hotelsDateTo: {$gte:hotelsDateTo[0]}}).toArray( function(err, results)
        {
            if(results.length>0){
                res.send(results);
            }
...
//console.log(hotelsDateFrom[0]);
    ...
    client.exists(hotelPlace+hotelsDateFrom[0]+hotelsDateTo[0], function(err, reply) {
         if(!err) {
             if (reply === 1) {
                 return res.status(201).send();
             } else {
                 var key = hotelPlace + hotelsDateFrom[0] + hotelsDateTo[0];
                 client.hmset(key, 'hotelsDateFrom', hotelsDateFrom[0], 'hotelsDateTo', hotelsDateTo[0], 'City', hotelPlace, function (error, result) {
                     if (!error) {
                         console.log('---------------')
                         console.log('Cached')
                         console.log('---------------')
                 ...
                 kafka.make_request('postHotelDetails', {
                     "City": hotelPlace,
                     "hotelsDateFrom": hotelsDateFrom[0],
                     "hotelsDateTo": hotelsDateTo[0]
